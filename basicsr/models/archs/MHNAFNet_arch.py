...
class MHNAFNet(nn.Module):

    def __init__(self, img_channel=3, width=16, middle_blk_num=1, enc_blk_nums=[], dec_blk_nums=[], n_heads=4, combinate_heads=False):
        super().__init__()
...
class MHNAFNetLocal(Local_Base, MHNAFNet):
    def __init__(self, *args, train_size=(1, 3, 256, 256), fast_imp=False, **kwargs):
        Local_Base.__init__(self)
        MHNAFNet.__init__(self, *args, **kwargs)
...
if __name__ == '__main__':
    ...
    using('start . ')
    net = MHNAFNet(img_channel=img_channel, width=width, middle_blk_num=middle_blk_num,
                      enc_blk_nums=enc_blks, dec_blk_nums=dec_blks, n_heads=4, combinate_heads=True)
